...
//페이지네이션 설정
        $config['base_url'] = base_url('/capp/index.php/board/lists/f_board/').$page_url.'/page/'; //페이징 주소
        $config['total_rows'] = $this->Board_m->get_list($this->uri->segment(3, 'f_board'), 'count', '', '', $search_word); //게시물의 전체 갯수
        $config['per_page'] = 15; //한 페이지에 표시할 게시물 수
        $config['uri_segment'] = $uri_segment; //페이지 번호가 위치한 세그먼트
        $config['first_url'] = base_url('/capp/index.php/board/lists/f_board/').$page_url.'/page/1'; //처음으로 url 설정
...
$this->load->helper('alert');
        ...
        if($this->session-> userdata('user') == TRUE) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (!$this->input->post('c_contents', TRUE)) {
                    alert("비정상적인 접근입니다. ", base_url('/capp/index.php/board/view/') . $this->uri->segment(3));
                    exit;
                }
                ...
                {
                    //글 작성 성공시 게시판 목록으로
                    alert('입력되었습니다.', base_url('/capp/index.php/board/view/') . $this->uri->segment(3)."/".$this->uri->segment(4));
                    exit;
                } else {
                    //글 실패 시 게시판 목록으로
                    alert('다시 입력해 주세요.', base_url('/capp/index.php/board/view/') . $this->uri->segment(3));
                    exit;
                }
            }
        } else {
		...
		$data['comment_list'] = $this ->Board_m ->get_comment($this->uri->segment(4));
	...
	function write()
	{
		...
		if($this->session-> userdata('user') == TRUE) {
			...
			$this->load->library('form_validation');
// 폼 검증할 필드와 규칙 사전 정의
			$this->form_validation->set_rules('b_title', '제목', 'required');
			$this->form_validation->set_rules('b_contents', '내용', 'required');
...
// 데이터베이스에 입력하고 그 결과를 돌려받음.
				...
				{
//글 작성 성공시 게시판 목록으로
					alert('입력되었습니다.', base_url('/capp/index.php/board/lists/') . $this->uri->segment(3));
					exit;
				} else {
					//글 실패 시 게시판 목록으로
					alert('다시 입력해 주세요.', base_url('/capp/index.php/board/lists/') . $this->uri->segment(3));
					exit;
				}
			} else {
